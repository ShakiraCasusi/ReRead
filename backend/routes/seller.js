...
// Require seller role for all routes below
...
router.get("/books", authenticateToken, async (req, res) => {
  try {
    ...
    let books = await Book.find({ sellerId: req.user.userId }).sort({
      createdAt: -1,
    });
      ...
      book.images.forEach((imageObj) => {
        ...
        if (imageUrl && imageUrl.includes(".s3.")) {
          try {
            ...
            console.warn(
              "Could not extract S3 key from images URL:",
              e.message,
            );
          }
        }
    ...
    if (book.bookFile) {
      ...
      if (fileUrl && fileUrl.includes(".s3.")) {
        ...
        try {
          ...
          console.warn(
            "Could not extract S3 key from bookFile URL:",
            e.message,
          );
        }
      }
