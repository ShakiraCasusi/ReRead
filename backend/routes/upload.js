...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Upload to S3
      const result = await s3Service.uploadToS3(req.file, "books/covers/", {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Upload to S3
      const result = await s3Service.uploadToS3(req.file, "users/profiles/", {
        "uploaded-by": req.user.id,
        "user-email": req.user.email,
      });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Upload to S3
      const result = await s3Service.uploadToS3(req.file, "books/files/", {
        "uploaded-by": req.user.id,
 ...
 */
router.get("/public-download/:fileKey", async (req, res) => {
  try {
    const { fileKey } = req.params;
