...
const S3_CONFIG = {
  ...
  allowedFileTypes: (
    process.env.ALLOWED_FILE_TYPES || "jpg,jpeg,png,gif,webp,pdf,epub,mobi"
  ).split(","),
...
if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
  console.warn(
    "⚠️  AWS credentials not configured. File uploads will not work.",
  );
  console.warn(
...
if (!process.env.AWS_S3_BUCKET_NAME) {
  console.warn(
    "⚠️  AWS_S3_BUCKET_NAME not configured. File uploads will not work.",
  );
}
