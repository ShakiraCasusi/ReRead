...
const bookValidators = {
  upsert: [
    ...
    body("isbn")
      .optional()
      .matches(
        /^(?:ISBN(?:-1[03])?:? )?(?=[-0-9 ]{10,}$|(?:(?=(?:[0-9]+[- ]){3})[- 0-9]{13}$)|(?:97[89][0-9]{10}$)|(?:(?=(?:[0-9]+[- ]){4})[- 0-9]{17}$)(?:(?<=^(?:ISBN(?:-1[03])?:? )?)97[89][- ]?|[- ]?(?!997[89]))[0-9]{2,5}[- ]?[0-9]+[- ]?[0-9]+[- ]?[0-9]$)/,
      )
...
const orderValidators = {
  create: [
    body("isDigitalOnly")
      .optional()
      .isBoolean()
      .withMessage("isDigitalOnly must be a boolean"),
    body("shippingAddress.street")
      .if((value, { req }) => !req.body.isDigitalOnly)
      .notEmpty()
      .trim()
