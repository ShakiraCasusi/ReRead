...
class AuthManager {
     ...
     * Save tokens
     */
    saveTokens(accessToken, refreshToken, expiresIn = 900000) { // 15 minutes default
    ...
    async refreshAccessToken() {
        ...
        try {
            ...
            if (data.success) {
                this.saveTokens(data.data.accessToken, data.data.refreshToken, data.data.expiresIn);
    ...
    createUserMenu() {
        ...
        const navEnd = navbar.querySelector('.navbar-collapse .ms-auto') || navbar.querySelector('.navbar-nav');
        if (!navEnd) return;
        ...
        try {
            ...
            if (data.success) {
                ...
                if (badge) {
                    ...
                    badge.style.display = data.data.itemCount > 0 ? 'inline-block' : 'none';
                }
            }
        } catch (error) {
